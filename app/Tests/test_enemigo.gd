...
const BolaEnemigo = preload("res://bola_enemigo.gd")
func pre_run():
	Global.reset_game()

func _contar_bolas(padre: Node) -> int:
	var cnt = 0
	for i in padre.get_child_count():
		if padre.get_child(i) is BolaEnemigo:
			cnt += 1
	return cnt
...
func test_disparo2_balas_tienen_direccion():
	...
	for i in range(padre.get_child_count()):
		...
		if hijo is BalaEnemigo:
			assert_true("direccion" in hijo, "Cada bala debe tener direccion")
			balas_validadas += 1
...
func test_disparar_siguiente_bala_crea_una():
	var enemigo = load("res://Scenes/enemigo.tscn").instantiate()
	var padre = Node2D.new()
	add_child_autoqfree(padre)
	padre.add_child(enemigo)
...
func test_disparar_siguiente_bala_tiene_direccion():
	...
	for i in range(padre.get_child_count()):
		var hijo = padre.get_child(i)
		if hijo is BalaEnemigo:
			assert_true(hijo.direccion != Vector2.ZERO,
				"La bala debe tener direccion distinta de cero")
			balas_validadas += 1
	assert_eq(balas_validadas, 1, "Se debió validar exactamente 1 bala")
func test_disparo3_tres_bolas_secuenciales():
	var enemigo = load("res://Scenes/enemigo.tscn").instantiate()
	var padre = Node2D.new()
	add_child_autoqfree(padre)
	padre.add_child(enemigo)
	...
	enemigo.disparo3()
	await get_tree().process_frame
	assert_eq(padre.get_child_count(), 2, "primera bola creada inmediatamente (en el siguiente frame)")
	await wait_seconds(0.5)
	assert_eq(padre.get_child_count(), 3, "segunda bola creada tras 0.5s")
	await wait_seconds(0.5)
	assert_eq(padre.get_child_count(), 4, "tercera bola creada tras 1.0s")
	await wait_seconds(0.5)
	assert_eq(padre.get_child_count(), 4, "no debe crear mas de 3 bolas")
